...
function nanoToMas(n: string | number | bigint) {
  const v = typeof n === "string" ? BigInt(n) : typeof n === "number" ? BigInt(n) : n;
...
type VaultState = {
  ...
  refetch: (vaultAddr?: string) => Promise<void>;
  refetchWithRetry: (vaultAddr?: string) => Promise<void>;
...
function applyVaultEvents(prev: { stats: Stats | null; user: User | null }, events: readonly any[]): { stats: Stats | null; user: User | null } {
  let stats = prev.stats ? { ...prev.stats } : null;
  let user = prev.user ? { ...prev.user } : null;

  console.debug("[Store] Applying vault events optimistically:", events?.length || 0);
  ...
  for (const e of events ?? []) {
    ...
    if (kind === "deposit") {
      ...
      if (stats) {
        console.debug(`[Store] WITHDRAW: TVL before: ${stats.tvlMas}, subtracting: ${mas}`);
        stats.tvlMas = Math.max(0, (stats.tvlMas ?? 0) - mas);
        stats.totalSharesNum = Math.max(0, (stats.totalSharesNum ?? 0) - Number(shares));
...
export const useVaultStore = create<VaultState>((set, get) => ({
  ...
  setVaultAddress: (address) => {
    set({ vaultAddress: address, stats: null, user: null, loading: createLoadingState() });
  },
  setLoading: (loading) => {
    set({ loading });
  },
  mutateAfterEvent: (events) => {
    ...
    console.debug(`[Store] Optimistic update at timestamp: ${lastOptimisticUpdate}`);
  ...
  refetchWithRetry: async (vaultAddr?: string) => {
    const targetVault = vaultAddr || get().vaultAddress;
    
    return await retryManager.executeWithRetry(
      `refetch-${targetVault}`,
      () => performanceMonitor.measure('refetch', () => get().refetch(vaultAddr)),
      {
        maxRetries: 3,
        baseDelay: 1000,
        maxDelay: 8000
  ...
  refetch: async (vaultAddr?: string) => {
    if (!ctx) return;
    const myV = ++version; // Capture version at start
    const targetVault = vaultAddr || get().vaultAddress;
    
    // Update loading state
    set({ loading: createLoadingState(true, 'refetch', 'Loading vault data...', 0) });
    
    console.debug(`[Store] Starting refetch version ${myV} for vault ${targetVault}`);
    ...
    try {
      // Update progress
      set({ loading: createLoadingState(true, 'refetch', 'Fetching contract data...', 25) });
      ...
      try {
        ...
        console.warn('[Store] Contract not ready yet, using defaults:', contractError);
        rawStats = JSON.stringify({
          tvl: "0", totalShares: "0", prizePool: "0", participants: "0",
          nextDrawPeriod: "0", drawPeriods: "5400", tickPeriods: "225",
          minPrizeThreshold: "100000000", winnerCount: "0", lastDrawPeriod: "0",
          contractVersion: "1.0.0", proposalCount: "0"
        });
        rawUser = JSON.stringify({ shares: "0", principal: "0", effectiveTickets: "0" });
      }
      
      // Update progress
      set({ loading: createLoadingState(true, 'refetch', 'Processing data...', 75) });

      // If a newer refetch started, drop this response
      if (myV !== version) {
        console.debug(`[Store] Dropping stale refetch ${myV}, current is ${version}`);
        return;
      }
      ...
      if (timeSinceOptimistic < 5000) { // 5 seconds grace period
        console.debug(`[Store] Refetch ${myV} might be stale (${timeSinceOptimistic}ms since optimistic), proceeding with caution`);
      }
      ...
      console.debug(`[Store] Raw TVL from contract: ${s.tvl}, converted: ${nanoToMas(s.tvl)}`);
      ...
      if (timeSinceOptimistic < graceTime && Math.abs(currentTvl - authoritativeTvl) > 0.01) {
        console.debug(`[Store] Authoritative TVL (${authoritativeTvl}) differs from optimistic (${currentTvl}), blockchain might be delayed`);
        console.debug(`[Store] Grace period: ${graceTime}ms, time since optimistic: ${timeSinceOptimistic}ms`);
