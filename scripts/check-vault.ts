...
async function checkVault() {
  try {
    ...
    if (!process.env.SECRET_KEY) {
      ...
      console.error('Make sure .env.local exists with SECRET_KEY=your_private_key');
      process.exit(1);
    }
    ...
    const currentPeriod = nodeStatus.currentCycle * 32 + nodeStatus.currentCycleTime; // Massa periods
    ...
    try {
      ...
      if (Number(position.shares) > 0) {
        ...
        console.log(`   Your Principal: ${Number(position.principal) / 1e9} MAS`);
        console.log(`   Effective Tickets: ${Number(position.effectiveTickets) / 1e9}`);
        
        if (Number(stats.totalShares) > 0) {
          const winChance = (Number(position.shares) / Number(stats.totalShares)) * 100;
    ...
    if (Number(stats.winnerCount) > 0) {
      ...
      winners.forEach((winner: any, index: number) => {
        console.log(`   #${index + 1}: ${winner.winner} won ${Number(winner.prize) / 1e9} MAS in period ${winner.period}`);
      });
    } else {
    ...
    if (prizePoolMAS >= minPrizeMAS && Number(stats.participants) > 0) {
      console.log(`   ✅ READY TO DRAW! Prize pool (${prizePoolMAS} MAS) >= minimum (${minPrizeMAS} MAS)`);
      ...
      console.log(`   ❌ NOT READY - Prize pool (${prizePoolMAS} MAS) < minimum (${minPrizeMAS} MAS)`);
      console.log(`   🎯 Need to fund prize pool with at least ${minPrizeMAS - prizePoolMAS} MAS`);
    }
